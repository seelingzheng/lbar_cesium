...
const locationComplete = (function() {
    ...
    function createCZMLBillboard({ name, center, label }) {
        return {
            id: name,
            name,
            billboard: {
            ...
            // },
            position: {
                cartographicDegrees: center,
            },
        };
    }

    function addCZMLData(czml) {
        if (czmlEntities) {
            viewer.dataSources.remove(czmlEntities);
        }

        // console.log(JSON.stringify(czml));

        Cesium.CzmlDataSource.load(czml).then((billboard) => {
            czmlEntities = billboard;
            viewer.dataSources.add(czmlEntities);
            viewer.zoomTo(czmlEntities.entities);
        });
    }
